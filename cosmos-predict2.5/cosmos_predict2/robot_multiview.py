...
def load_agibot_camera_fn():
    ...
    def _resolve_camera_txt(base_path: str, input_name: str, cam_type: str) -> str:
        primary = os.path.join(base_path, "cameras", f"{input_name}_{cam_type}.txt")
        if os.path.exists(primary):
            return primary
        # Convenience alias: allow users to name the main view "low" instead of "head".
        if cam_type.endswith("_head"):
            alt = os.path.join(base_path, "cameras", f"{input_name}_{cam_type[:-5]}_low.txt")
    ...
    def load_fn(
        ...
        # `path` is like: {base_path}/input_images/{input_name}
        # Use the actual input_name (supports non-numeric and zero-padded names, per robot_multiview_config.py docs).
        input_name = Path(path).name
        data = {"text": text, "video": visual, "path": path}
        extrinsics_list = []
        for cam_type in cam_data_list:
            extrinsics_tgt = torch.tensor(
                np.loadtxt(_resolve_camera_txt(base_path, input_name, cam_type))
            ).to(torch.bfloat16)
        ...
        for intrinsic_type in intrinsic_data_list:
            intrinsics_tgt = torch.tensor(
                np.loadtxt(_resolve_camera_txt(base_path, input_name, intrinsic_type))
            ).to(torch.bfloat16)
...
class TextImageCameraDataset(torch.utils.data.Dataset):
    ...
    def load_images(self, input_name: str) -> torch.Tensor:
        ...
        for image_name in images_name:
            ...
            if not os.path.exists(image_path) and image_name == "head":
                # Allow naming the main view as "low" (e.g., <input_name>_low.png).
                image_path = os.path.join(self.base_path, "input_images", input_name + "_low.png")
